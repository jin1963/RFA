...
async function connectWallet() {
  ...
  try {
    ...
    usdtDecimals = await getTokenDecimals(usdtContract, 18); function generateReferralLink() {
  if (!account) {
...
window.ethereum?.on('chainChanged', (chainId) => {
    ...
    if (receipt && receipt.status) {
        ...
        document.getElementById("buyTokenStatus"].classList.add("success");
        loadStakingInfo();
        loadReferralInfo(); // **สำคัญ:** เรียก loadReferralInfo() ด้วย
    } else {
